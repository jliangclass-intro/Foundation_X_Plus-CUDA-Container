...
# All rights reserved. ##
"""Dataset and augmentation utilities for Foundation_X+.

Albumentations compatibility note:
- Newer albumentations releases removed ``IAAAdditiveGaussianNoise``.
- This module uses ``GaussNoise`` instead to stay compatible with current
    package versions.
- Legacy transforms such as ``RandomBrightness``/``RandomContrast`` are
    avoided in favor of ``RandomBrightnessContrast``.
"""
...
def _resolve_image_path(path: str) -> str:
    """Resolve an image path and fall back to common extension variants.

    This handles datasets where list files point to ``.jpeg`` but files are
    actually stored as ``.jpg`` (or vice versa), including uppercase variants.
    """
    ...
    tried = [str(image_path)]
    base_path = image_path.with_suffix("") if image_path.suffix else image_path
    ...
    raise FileNotFoundError(
        f"Image not found for path '{path}'. Tried extension fallbacks: {tried[:8]}"
    )
...
def build_ts_transformations():
    AUGMENTATIONS = Compose(
        [
            # RandomResizedCrop(height=224, width=224, scale=(0.90,1.0)),
            # TODO: hard coded 224x224, but, will need to be updated via command argument
            RandomResizedCrop(size=(224, 224), scale=(0.90, 1.0)),
...
class PXSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        imageData = cv2.resize(
            cv2.imread(imagePath, cv2.IMREAD_COLOR),
            self.image_size,
            interpolation=cv2.INTER_AREA,
        )
        maskData = cv2.resize(
            cv2.imread(maskPath, cv2.IMREAD_GRAYSCALE),
            self.image_size,
            interpolation=cv2.INTER_AREA,
        )
...
class MontgomeryDataset(Dataset):
            ...
            with open(pathDatasetFile, "r") as fileDescriptor:
                ...
                while line:
                    ...
                    self.img_label.append(
                        (
                            join(pathImageDirectory + "/ManualMask/leftMask", line),
                            (join(pathImageDirectory + "/ManualMask/rightMask", line)),
                        )
                    )
                    line = fileDescriptor.readline().strip()
    ...
    def __getitem__(self, idx):
        ...
        leftMaskData = cv2.resize(
            cv2.imread(maskPath[0], cv2.IMREAD_GRAYSCALE),
            self.image_size,
            interpolation=cv2.INTER_AREA,
        )
        rightMaskData = cv2.resize(
            cv2.imread(maskPath[1], cv2.IMREAD_GRAYSCALE),
            self.image_size,
            interpolation=cv2.INTER_AREA,
        )
            ...
            with open(pathDatasetFile, "r") as fileDescriptor:
                ...
                while line:
                    self.img_list.append(
                        join(pathImageDirectory + "/images", line + ".IMG.png")
                    )
                    self.img_label.append(
                        (
                            join(
                                pathImageDirectory + "/masks/left_lung_png",
                                line + ".png",
                            )
                        )
                    )
                ...
                while line:
                    ...
                    self.img_label.append(
                        (
                            ...
                            (
                                join(
                                    pathImageDirectory + "/masks/right_lung_png",
                                    line + ".png",
                                )
                            ),
                        )
            ...
            with open(pathDatasetFile, "r") as fileDescriptor:
                ...
                while line:
                    ...
                    self.img_label.append(
                        (
                            join(
                                pathImageDirectory + "/masks/left_clavicle_png/",
                                line + ".png",
                            ),
                            (
                                join(
                                    pathImageDirectory + "/masks/right_clavicle_png/",
                                    line + ".png",
                                )
                            ),
                        )
        ...
        for pathImageDirectory, pathDatasetFile in image_path_file:
            with open(pathDatasetFile, "r") as fileDescriptor:
                ...
                while line:
                    ...
                    self.img_label.append(
                        join(pathImageDirectory + "/masks/heart_png/", line + ".png")
                    )
                    line = fileDescriptor.readline().strip()
...
class VindrCXRHLLRL_3chDataset(
    Dataset
):  ## Segmentation Dataloader for 1H 3Ch Heart-LeftLung-RightLung
    def __init__(self, image_path_file, image_size=(448, 448), mode="train", ann=-1):
        ...
        for pathImageDirectory, pathDatasetFile in image_path_file:
            with open(pathDatasetFile, "r") as fileDescriptor:
                ...
                while line:
                    self.img_list.append(
                        join(pathImageDirectory + mode_images, line + ".jpeg")
                    )
                    self.img_label.append(
                        (
                            join(pathImageDirectory + mode_masksL, line + ".png"),
                            (join(pathImageDirectory + mode_masksR, line + ".png")),
                            (join(pathImageDirectory + mode_masksH, line + ".png")),
                        )
                    )
                    line = fileDescriptor.readline().strip()

        if (
            self.mode == "train" and self.ann == 7500
        ):  ## last 7500 data for segmentation and first to 7500 is for localization
            self.img_list.sort()
            self.img_label.sort()
    ...
    def __getitem__(self, idx):
        ...
        heartMaskData = cv2.resize(
            cv2.imread(maskPath[2], cv2.IMREAD_GRAYSCALE),
            self.image_size,
            interpolation=cv2.INTER_AREA,
        )
...
class VindrCXRLeftLungDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train" or self.mode == "trainA" or self.mode == "trainB":
            ...
            dic = self.transformSequence[self.mode](
                image=imageData, mask=maskData
            )  # TEST
        img = dic["image"]
        mask = dic["mask"]
        ...
        for pathImageDirectory, pathDatasetFile in image_path_file:
            with open(pathDatasetFile, "r") as fileDescriptor:
                ...
                while line:
                    ...
                    self.img_label.append(
                        join(pathImageDirectory + mode_masks, line + ".png")
                    )
                    line = fileDescriptor.readline().strip()
        if self.mode == "train" and self.ann == 2500:
            self.img_list.sort()
...
class chestxdet_dataset(Dataset):  # only Segmentation ## From Anni
    ...
    def combine_mask(self, img, masks_list):
        combined_mask = np.zeros(
            (self.image_size[0], self.image_size[1], len(masks_list)), dtype=np.uint8
        )
...
class ChestXDet_cls(Dataset):  ## For Classification
    def __init__(
        self, images_path, file_path, augment, num_class=13, anno_percent=100
    ):  ## ChestX_det_train_NAD_v2  ChestX_det_test_NAD_v2
        self.img_list = []
        self.img_label = []
        self.disease_labels = [
            "Atelectasis",
            ...
            while line:
                ...
                if line:
                    ...
                    imagePath = os.path.join(
                        images_path, split_folder, lineItems[0] + ".dcm.jpeg"
                    )
...
class ShenzhenCXR(Dataset):
    def __init__(
        ...
        if annotation_percent < 100:
            ...
            _img_list, _img_label = (
                copy.deepcopy(self.img_list),
                copy.deepcopy(self.img_label),
            )
            self.img_list = []
            self.img_label = []
...
class VinDrRibCXRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        imageData = cv2.imread(
            os.path.join(self.pathImageDirectory, imagePath), cv2.IMREAD_COLOR
        )
        label0 = []
...
class NODE21(Dataset):
    def __init__(
        self, images_path, file_path, augment, num_class=2, annotation_percent=100
    ):
        self.img_list = []
        self.img_label = []
        self.augment = augment
            ...
            while line:
                ...
                if line:
                    ...
                    if images_path_has_png_dir:
                        ...
                        imagePath = os.path.join(
                            images_path, "png_images", lineItems[0]
                        )
                    imageLabel = int(lineItems[1])
...
class VinDrCXR(Dataset):
    ...
    def __getitem__(self, index):
        ...
        imageLabel = torch.FloatTensor(self.img_label[index])
        imageData = (
            Image.open(_resolve_image_path(imagePath)).convert("RGB").resize((224, 224))
        )
...
class SIIMPneumothorax_test_original(Dataset):
    def __init__(self, images_path, image_file, image_size=(768, 768)):

        self.img_list = []
        self.image_size = image_size
        sample_submission = pd.read_csv(image_file)
        image_ids = sample_submission["ImageId"].tolist()

        for img_id in image_ids:
            self.img_list.append(os.path.join(images_path, img_id + ".dcm"))

        print("Total test images: ", len(self.img_list))
    ...
    def __getitem__(self, idx):
        ...
        im_array = dicom.dcmread(imagePath).pixel_array
        imageData = Image.fromarray(im_array).convert("RGB")
        imageData = imageData.resize(self.image_size)
...
class TBX11KDataset(Dataset):  ## Need to FIX
    def __init__(
        ...
        with open(os.path.join(images_path, file_path), "r") as fileDescriptor:
            line = True
            while line:
                line = fileDescriptor.readline()
        ...
        with open(file_path, "r") as fileDescriptor:
            ...
            for line in csvReader:
                ...
                if self.mode == "train":
                    ...
                    label = line[1:]
                # print("imagePath:", imagePath, line)
                for i in range(num_class):
                    if label[i]:
                        a = float(label[i])
                        if a == 1:
...
# Nate's Objects365 dataloader (deprecated)
class Objects365Dataset(Dataset):
    def __init__(self, annotations_file, img_dir, transform=None):
        ...
        self.transforms = transforms.Compose(
            [
                transforms.ToTensorV2(),  # why V2?
            ]
        )

        # extract full image paths
        for img_info in data["images"]:
            img_id = img_info["id"]
            self.img_list.append(img_id)
            path = img_info["file_name"]
            # fix the path to match file structure as in /jliang12/data/objects365
            path = path.replace("images/v2/", "").replace("images/v1/", "")
            self.img_paths[img_id] = path

        # extract annotations for each image
        for ann in data["annotations"]:
            img_id = ann["image_id"]
            if img_id not in self.img_label:
                self.img_label[img_id] = []
            self.img_label[img_id].append(ann)
    ...
    def __getitem__(self, idx):
        ...
        # load image
        img_path = os.path.join(self.img_dir, self.img_paths[img_id])
        image = Image.open(img_path).convert("RGB")

        # get annotations for the image
        annotations = self.img_label[img_id]
...
class COCOSegmentationDataset(Dataset):
    def __init__(self, root_dir, annotation_dir, image_size=(224, 224), mode="train"):
        """
        Args:
            root_dir (string): Directory with all the images.
            annotation_dir (string): Directory with all the masks.
        """
        ...
        # List all the images and masks
        self.image_files = sorted(
            [f for f in os.listdir(root_dir) if f.endswith(".jpg")]
        )
        print(f"annotation_dir: {annotation_dir}")
        self.mask_files = sorted(
            [f for f in os.listdir(annotation_dir) if f.endswith(".png")]
        )
        print(f"img len: {len(self.image_files)}")
        print(f"mask len: {len(self.mask_files)}")

        # img, mask = self.__getitem__(1)
        # self.save_image_and_mask(img, mask)
    ...
    def __getitem__(self, idx):
        ...
        mask_name = os.path.join(self.annotation_dir, self.mask_files[idx])

        # print("[Dataloader Info.] Image:", img_name)
        # print("[Dataloader Info.] Mask:", mask_name)

        imageData = cv2.resize(
            cv2.imread(img_name, cv2.IMREAD_COLOR),
            self.image_size,
            interpolation=cv2.INTER_AREA,
        )
        # print(f"[Debug Info.] Image dimensions BEFORE rearrange: {imageData.shape}")
        imageData = rearrange(imageData, "h w c-> c h w") / 255
        # print(f"[Debug Info.] Image dimensions AFTER rearrange: {imageData.shape}")
...
class ADE20KDataset(BaseDataSet):
    ### ADE20K dataset 
    ### adapted from http://groups.csail.mit.edu/vision/datasets/ADE20K/
    ...
    def _set_files(self):
        if self.split in  ["training", "valid"]:
...
def dataloader_return(args):
    ...
    if (
        args.taskcomponent == "segmentation"
        or args.taskcomponent == "segmentation_cyclic"
        or args.taskcomponent == "segmentation_vindrcxr_organ_1h3ch"
    ):
        # print("CHECK!!!")
        if args.segmentation_dataset == "jsrt_lung":
            # Original SOL path: /data/jliang12/jpang12/dataset/JSRT/All247images
            jsrt_root = _dataset_location(
                "directories",
                "jsrt_all247images",
                default="/scratch/sejong/class-dataset/JSRT/All247images",
            )
            train_image_path_file = [(jsrt_root, "data/jsrt/train.txt")]
            val_image_path_file = [(jsrt_root, "data/jsrt/val.txt")]
            test_image_path_file = [(jsrt_root, "data/jsrt/test.txt")]

            train_dataset = JSRTLungDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize)
            )
            ...
            val_dataset = JSRTLungDataset(
                val_image_path_file, image_size=(args.imgsize, args.imgsize), mode="val"
            )
            ...
            test_dataset = JSRTLungDataset(
                test_image_path_file,
            ...
            train_dataset = JSRTClavicleDataset(
                train_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="train",
                ann=args.segmentation_dataset_ann,
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
            ...
            val_dataset = JSRTClavicleDataset(
                val_image_path_file, image_size=(args.imgsize, args.imgsize), mode="val"
            )
            val_loader = torch.utils.data.DataLoader(
                val_dataset,
            ...
            test_dataset = JSRTClavicleDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="val",
            )
            ...
            train_dataset = JSRTHeartDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize)
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
            ...
            train_dataset = JSRTLeftLungDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize)
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
            ...
            val_dataset = JSRTLeftLungDataset(
                val_image_path_file, image_size=(args.imgsize, args.imgsize), mode="val"
            )
            val_loader = torch.utils.data.DataLoader(
                val_dataset,
            ...
            test_dataset = JSRTLeftLungDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="val",
            )
            ...
            if args.serverC == "DFS":
                train_image_path_file = [
                    (
                        "/mnt/dfs/nuislam/Data/ChestX-Det",
                        "data/chestxdetdataset/ChestX-Det_train_data.txt",
                    )
                ]
                ...
                test_image_path_file = [
                    (
                        "/mnt/dfs/nuislam/Data/ChestX-Det",
                        "data/chestxdetdataset/ChestX-Det_test_data.txt",
                    )
                ]
            elif args.serverC == "SOL":
                train_image_path_file = [
                    (
                        "/scratch/jliang12/data/ChestX-Det",
                        "data/chestxdetdataset/ChestX-Det_train_data.txt",
                    )
                ]
                # val_image_path_file = [("/scratch/jliang12/data/ChestX-Det", "/scratch/jliang12/data/ChestX-Det/data_files/ChestX-Det_valid_data.txt")]
            ...
            train_loader_jsrtLung = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            val_loader_jsrtLung = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            test_loader_jsrtLung = torch.utils.data.DataLoader(
                test_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            train_loader_jsrtClavicle = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            val_loader_jsrtClavicle = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            test_loader_jsrtClavicle = torch.utils.data.DataLoader(
                test_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            train_loader_jsrtHeart = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            val_loader_jsrtHeart = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            test_loader_jsrtHeart = torch.utils.data.DataLoader(
                test_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            return (
                train_loader_jsrtLung,
                val_loader_jsrtLung,
                test_loader_jsrtLung,
                train_loader_jsrtClavicle,
                val_loader_jsrtClavicle,
                test_loader_jsrtClavicle,
                train_loader_jsrtHeart,
                val_loader_jsrtHeart,
                test_loader_jsrtHeart,
            )
            ...
            if args.serverC == "DFS":
                train_image_path_file = [
                    (
                        "/mnt/dfs/jpang12/datasets/VinDr-CXR/physionet.org/files/vindr-cxr/1.0.0",
                        "data/vindrcxr_organ_segmentation/vindrcxr_organSegmentation_train.txt",
                    )
                ]
                test_image_path_file = [
                    (
                        "/mnt/dfs/jpang12/datasets/VinDr-CXR/physionet.org/files/vindr-cxr/1.0.0",
                        "data/vindrcxr_organ_segmentation/vindrcxr_organSegmentation_test.txt",
                    )
                ]
            elif args.serverC == "SOL":
                train_image_path_file = [
                    (
                        "/scratch/jliang12/data/VinDr-CXR/physionet.org/files/vindr-cxr/1.0.0",
                        "data/vindrcxr_organ_segmentation/vindrcxr_organSegmentation_train.txt",
                    )
                ]
                test_image_path_file = [
            ...
            train_dataset = VindrCXRLungDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize)
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
            ...
            val_dataset = VindrCXRLungDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="test",
            )
            ...
            train_dataset = VindrCXRHeartDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize), ann=2500
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
            ...
            val_dataset = VindrCXRHeartDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="test",
            )
            ...
            train_dataset = VindrCXRLeftLungDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize), ann=2500
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
            ...
            val_dataset = VindrCXRLeftLungDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="test",
            )
            ...
            train_dataset = VindrCXRRightLungDataset(
                train_image_path_file, image_size=(args.imgsize, args.imgsize), ann=2500
            )
            train_loader = torch.utils.data.DataLoader(
                train_dataset,
            ...
            val_dataset = VindrCXRRightLungDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="test",
            )
            ...
            train_loader_vindrcxrHeart = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            val_loader_vindrcxrtHeart = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            train_loader_vindrcxrLung = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            val_loader_vindrcxrtLung = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            return (
                train_loader_vindrcxrHeart,
                val_loader_vindrcxrtHeart,
                train_loader_vindrcxrLung,
                val_loader_vindrcxrtLung,
            )

        elif (
            args.segmentation_dataset == "segmentation_vindrcxr_organ_1h3ch"
        ):  # segmentation_vindrcxr_organ_1h3ch
            ...
            train_dataset = VindrCXRHLLRL_3chDataset(
                train_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="train",
                ann=7500,
            )
            train_loader_vindrcxr3ch = torch.utils.data.DataLoader(
                train_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            val_dataset = VindrCXRHLLRL_3chDataset(
                test_image_path_file,
                image_size=(args.imgsize, args.imgsize),
                mode="test",
            )
            val_loader_vindrcxrt3ch = torch.utils.data.DataLoader(
                val_dataset,
                batch_size=args.batch_size,
                num_workers=args.num_workers,
                pin_memory=True,
    ...
    if args.taskcomponent == "detect_segmentation_cyclic":
        ...
        val_dataset = VindrCXRHeartDataset(
            test_image_path_file, image_size=(args.imgsize, args.imgsize), mode="test"
        )
        val_loader_vindrcxrtHeart = torch.utils.data.DataLoader(
            val_dataset,
            batch_size=args.batch_size,
    ...
    if (
        ...
        or args.taskcomponent == "detect_segmentation_cyclic_v3"
    ):  ## with EMA ## Training Strategy 1 OR Training Strategy 2
        ### SEGMENTATION DATA LOADING
        if args.serverC == "DFS":
            train_image_path_file = [
                (
        ...
        train_dataset = VindrCXRHeartDataset(
            ...
            mode="train",
            ann=2500,
        )  # trainA/B 1250 | train 2500
        train_loader_vindrcxrHeart_A = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        train_loader_vindrcxrLeftLung_A = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        val_loader_vindrcxrtLeftLung = torch.utils.data.DataLoader(
            val_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        train_loader_vindrcxrRightLung_A = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        val_loader_vindrcxrtRightLung = torch.utils.data.DataLoader(
            val_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        ## Heart
        dataset_trainHeartA = build_dataset(
            image_set="vindrcxrOrgan_trainHeart", args=args
        )  # vindrcxrOrgan_trainHeart | vindrcxrOrgan_trainHeartA1250
        ...
        batch_sampler_trainHeartA = torch.utils.data.BatchSampler(
            sampler_trainHeartA, args.batch_size, drop_last=True
        )
        # batch_sampler_trainHeartB = torch.utils.data.BatchSampler(sampler_trainHeartB, args.batch_size, drop_last=True)
        data_loader_trainHeartA = DataLoader(
            dataset_trainHeartA,
            batch_sampler=batch_sampler_trainHeartA,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        ...
        data_loader_valHeart = DataLoader(
            dataset_valHeart,
            args.batch_size,
            sampler=sampler_valHeart,
            drop_last=False,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        ...
        ## Left Lung
        dataset_trainLeftLungA = build_dataset(
            image_set="vindrcxrOrgan_trainLeftLung", args=args
        )  # vindrcxrOrgan_trainLeftLung | vindrcxrOrgan_trainLeftLungA1250
        ...
        dataset_valLeftLung = build_dataset(
            image_set="vindrcxrOrgan_testLeftLung", args=args
        )
        if args.distributed:
            ...
            sampler_trainLeftLungA = torch.utils.data.RandomSampler(
                dataset_trainLeftLungA
            )
            # sampler_trainLeftLungB = torch.utils.data.RandomSampler(dataset_trainLeftLungB)
            sampler_valLeftLung = torch.utils.data.SequentialSampler(
                dataset_valLeftLung
            )
        batch_sampler_trainLeftLungA = torch.utils.data.BatchSampler(
            sampler_trainLeftLungA, args.batch_size, drop_last=True
        )
        # batch_sampler_trainLeftLungB = torch.utils.data.BatchSampler(sampler_trainLeftLungB, args.batch_size, drop_last=True)
        data_loader_trainLeftLungA = DataLoader(
            dataset_trainLeftLungA,
            batch_sampler=batch_sampler_trainLeftLungA,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        ...
        data_loader_valLeftLung = DataLoader(
            dataset_valLeftLung,
            args.batch_size,
            sampler=sampler_valLeftLung,
            drop_last=False,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        ...
        ## Right Lung
        dataset_trainRightLungA = build_dataset(
            image_set="vindrcxrOrgan_trainRightLung", args=args
        )  # vindrcxrOrgan_trainRightLung | vindrcxrOrgan_trainRightLungA1250
        ...
        dataset_valRightLung = build_dataset(
            image_set="vindrcxrOrgan_testRightLung", args=args
        )
        if args.distributed:
            ...
            sampler_valRightLung = DistributedSampler(
                dataset_valRightLung, shuffle=False
            )
        else:
            sampler_trainRightLungA = torch.utils.data.RandomSampler(
                dataset_trainRightLungA
            )
            # sampler_trainRightLungB = torch.utils.data.RandomSampler(dataset_trainRightLungB)
            sampler_valRightLung = torch.utils.data.SequentialSampler(
                dataset_valRightLung
            )
        batch_sampler_trainRightLungA = torch.utils.data.BatchSampler(
            sampler_trainRightLungA, args.batch_size, drop_last=True
        )
        # batch_sampler_trainRightLungB = torch.utils.data.BatchSampler(sampler_trainRightLungB, args.batch_size, drop_last=True)
        data_loader_trainRightLungA = DataLoader(
            dataset_trainRightLungA,
            batch_sampler=batch_sampler_trainRightLungA,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        ...
        data_loader_valRightLung = DataLoader(
            dataset_valRightLung,
            args.batch_size,
            sampler=sampler_valRightLung,
            drop_last=False,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        ...
        return (
            train_loader_vindrcxrHeart_A,
            val_loader_vindrcxrtHeart,
            train_loader_vindrcxrLeftLung_A,
            val_loader_vindrcxrtLeftLung,
            train_loader_vindrcxrRightLung_A,
            val_loader_vindrcxrtRightLung,
            val_loader_vindrcxrt3ch,
            data_loader_trainHeartA,
            sampler_trainHeartA,
            data_loader_valHeart,
            dataset_valHeart,
            data_loader_trainLeftLungA,
            sampler_trainLeftLungA,
            data_loader_valLeftLung,
            dataset_valLeftLung,
            data_loader_trainRightLungA,
            sampler_trainRightLungA,
            data_loader_valRightLung,
            dataset_valRightLung,
        )
    ...
    if args.taskcomponent == "detect_segmentation_cyclic_v4":  ## with EMA
        ...
        train_loader_vindrcxrLeftLung = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        train_loader_vindrcxrRightLung = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        ## Heart
        dataset_trainHeart = build_dataset(
            image_set="vindrcxrOrgan_trainHeart", args=args
        )
        dataset_valHeart = build_dataset(image_set="vindrcxrOrgan_testHeart", args=args)
        if args.distributed:
            ...
            sampler_valHeart = torch.utils.data.SequentialSampler(dataset_valHeart)
        batch_sampler_trainHeart = torch.utils.data.BatchSampler(
            sampler_trainHeart, args.batch_size, drop_last=True
        )
        data_loader_trainHeart = DataLoader(
            dataset_trainHeart,
            batch_sampler=batch_sampler_trainHeart,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        data_loader_valHeart = DataLoader(
        ...
        ## Left Lung
        dataset_trainLeftLung = build_dataset(
            image_set="vindrcxrOrgan_trainLeftLung", args=args
        )
        dataset_valLeftLung = build_dataset(
            image_set="vindrcxrOrgan_testLeftLung", args=args
        ...
        if args.distributed:
            ...
            sampler_trainLeftLung = torch.utils.data.RandomSampler(
                dataset_trainLeftLung
            )
            sampler_valLeftLung = torch.utils.data.SequentialSampler(
                dataset_valLeftLung
            )
        batch_sampler_trainLeftLung = torch.utils.data.BatchSampler(
            sampler_trainLeftLung, args.batch_size, drop_last=True
        )
        data_loader_trainLeftLung = DataLoader(
            dataset_trainLeftLung,
            batch_sampler=batch_sampler_trainLeftLung,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        data_loader_valLeftLung = DataLoader(
        ...
        ## Right Lung
        dataset_trainRightLung = build_dataset(
            image_set="vindrcxrOrgan_trainRightLung", args=args
        )
        dataset_valRightLung = build_dataset(
            image_set="vindrcxrOrgan_testRightLung", args=args
        ...
        if args.distributed:
            ...
            sampler_trainRightLung = torch.utils.data.RandomSampler(
                dataset_trainRightLung
            )
            sampler_valRightLung = torch.utils.data.SequentialSampler(
                dataset_valRightLung
            )
        batch_sampler_trainRightLung = torch.utils.data.BatchSampler(
            sampler_trainRightLung, args.batch_size, drop_last=True
        )
        data_loader_trainRightLung = DataLoader(
            dataset_trainRightLung,
            batch_sampler=batch_sampler_trainRightLung,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        data_loader_valRightLung = DataLoader(
        ...
        return (
            train_loader_vindrcxrHeart,
            val_loader_vindrcxrtHeart,
            train_loader_vindrcxrLeftLung,
            val_loader_vindrcxrtLeftLung,
            train_loader_vindrcxrRightLung,
            val_loader_vindrcxrtRightLung,
            data_loader_trainHeart,
            sampler_trainHeart,
            data_loader_valHeart,
            dataset_valHeart,
            data_loader_trainLeftLung,
            sampler_trainLeftLung,
            data_loader_valLeftLung,
            dataset_valLeftLung,
            data_loader_trainRightLung,
            sampler_trainRightLung,
            data_loader_valRightLung,
            dataset_valRightLung,
        )

    if args.taskcomponent in [
        "detect_vindrcxr_heart_segTest",
        "detect_vindrcxr_heart",
        "detect_vindrcxr_leftlung",
        "detect_vindrcxr_rightlung",
    ]:
        ### Localization Data Loading
    ...
    if args.taskcomponent == "classification":
        if args.classification_dataset == "imagenet":
            ...
            if args.train:
                ...
                traindir = os.path.join(
                    "/scratch/jliang12/data/ImageNet", "train"
                )  # /data/jliang12/rfeng12/Data/ImageNet  /scratch/jliang12/data/ImageNet
            ...
            valdir = os.path.join(
                "/scratch/jliang12/data/ImageNet", "val"
            )  # /data/jliang12/rfeng12/Data/ImageNet  /scratch/jliang12/data/ImageNet
            val_loader = torch.utils.data.DataLoader(
        ...
        if args.classification_dataset == "ChestXray14":
            ...
            # Original SOL path: /data/jliang12/jpang12/dataset/nih_xray14/images/images/
            data_dir = _dataset_location(
                "directories",
                "nih_chestxray14_images",
                default=DATASETS_CONFIG["cls_nih_root"],
            )

            dataset_train = ChestXray14Dataset(
            ...
            dataset_val = ChestXray14Dataset(
                images_path=data_dir,
                file_path=val_list,
                augment=build_transform_classification(
    ...
    if args.taskcomponent in ["detect_node21_nodule"]:
        ...
        if args.serverC == "DFS":
            ...
            data_dir = "/scratch/jliang12/data/NODE21/"

        dataset_train = NODE21(
            images_path=data_dir,
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
        ...
        dataset_test = NODE21(
            ...
            file_path=test_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="test2"
            ),
        )
    ...
    if args.taskcomponent in ["ClsLoc_tbx11k_catagnostic"]:
        ...
        if args.serverC == "DFS":
            ...
            data_dir = "/scratch/jliang12/data/tbx11k/TBX11K/"
    ...
    if args.taskcomponent in ["detection_vindrcxr_disease"]:
        ...
        dataset_train = VindrCXRClass(
            images_path=os.path.join(data_dir, "train_jpeg"),
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
            ),
        ...
        dataset_test = VindrCXRClass(
            images_path=os.path.join(data_dir, "test_jpeg"),
            file_path=test_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="test2"
            ),
    ...
    if args.taskcomponent in ["detection_vindrmammo_disease"]:
        ...
        dataset_train = VindrmammoClass(
            images_path=os.path.join(data_dir, "images_png"),
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
            ),
    ...
    if args.taskcomponent in ["detect_chestxdet_dataset"]:
        ...
        train_loader_seg = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        test_loader_seg = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        if args.serverC == "DFS":
            ...
            train_list = (
                "/scratch/jliang12/data/ChestX-Det/ChestX_det_train_NAD_v2.json"
            )
            test_list = "/scratch/jliang12/data/ChestX-Det/ChestX_det_test_NAD_v2.json"

        dataset_train = ChestXDet_cls(
            images_path=data_dir,
            file_path="train",
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
    ...
    if args.taskcomponent in ["foundation_x_pretraining"]:
        ...
        train_loader_cls_TBX11k = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_TBX11k = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        train_loader_cls_NODE21 = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_NODE21 = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        train_loader_cls_ChestXDet = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_ChestXDet = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        dataset_train = RSNAPneumonia(
            images_path="/scratch/jliang12/data/rsna-pneumonia-detection-challenge/stage_2_train_images_png/",
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
            ),
        )
        dataset_test = RSNAPneumonia(
            images_path="/scratch/jliang12/data/rsna-pneumonia-detection-challenge/stage_2_test_images_png/",
            file_path=test_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="test2"
            ),
        )
        train_loader_cls_RSNApneumonia = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_RSNApneumonia = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        dataset_train = SIIMPTX(
            images_path="/scratch/jliang12/data/siim_pneumothorax_segmentation/",
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
            ),
        )
        dataset_test = SIIMPTX(
            images_path="/scratch/jliang12/data/siim_pneumothorax_segmentation/",
            file_path=test_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="test2"
        ...
        train_loader_cls_SIIMACRptx = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_SIIMACRptx = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        dataset_train = CANDIDPTX(
            images_path="/scratch/jliang12/data/CANDID-PTX/dataset/",
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", mode="train"
            ),
        ...
        ## Dataloader for Localization -------------------------------------------------------------
        ## -- TBX11K Dataset -- ##
        dataset_train_loc_TBX11k = build_dataset(
            image_set="tbx11k_catagnostic_train_A", args=args
        )  ## TBX11K Dataset - Training A set # tbx11k_catagnostic_train | tbx11k_catagnostic_train_A | tbx11k_catagnostic_train_B
        dataset_val_loc_TBX11k = build_dataset(
            image_set="tbx11k_catagnostic_test", args=args
        )
        if args.distributed:
            ...
            sampler_train_TBX11k = torch.utils.data.RandomSampler(
                dataset_train_loc_TBX11k
            )
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_loc_TBX11k)
            # sampler_test = torch.utils.data.SequentialSampler(dataset_test) # added by Nahid
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_TBX11k, args.batch_size, drop_last=True
        )
        train_loader_loc_TBX11k = DataLoader(
            dataset_train_loc_TBX11k,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )  # RETURN
        test_loader_loc_TBX11k = DataLoader(
            dataset_val_loc_TBX11k,
            1,
            sampler=sampler_val,
            drop_last=False,
            collate_fn=utils.collate_fn,
        ...
        dataset_train_loc_TBX11k_B = build_dataset(
            image_set="tbx11k_catagnostic_train_B", args=args
        )  ## TBX11K Dataset - Training B set
        if args.distributed:
            sampler_train_TBX11k_B = DistributedSampler(dataset_train_loc_TBX11k_B)
        else:
            sampler_train_TBX11k_B = torch.utils.data.RandomSampler(
                dataset_train_loc_TBX11k_B
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_TBX11k_B, args.batch_size, drop_last=True
        )
        train_loader_loc_TBX11k_B = DataLoader(
            dataset_train_loc_TBX11k_B,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        ...
        ## -- NODE21 Dataset -- ##
        dataset_train_loc_Node21 = build_dataset(
            image_set="node21_noduleDataset_train_A", args=args
        )  ## NODE21 Dataset - Training A set # node21_noduleDataset_train | node21_noduleDataset_train_A | node21_noduleDataset_train_B
        dataset_val_loc_Node21 = build_dataset(
            image_set="node21_noduleDataset_test", args=args
        )
        if args.distributed:
            ...
            sampler_train_Node21 = torch.utils.data.RandomSampler(
                dataset_train_loc_Node21
            )
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_loc_Node21)
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_Node21, args.batch_size, drop_last=True
        )
        train_loader_loc_Node21 = DataLoader(
            dataset_train_loc_Node21,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        test_loader_loc_Node21 = DataLoader(
            dataset_val_loc_Node21,
            1,
            sampler=sampler_val,
            drop_last=False,
            collate_fn=utils.collate_fn,
        ...
        dataset_train_loc_Node21_B = build_dataset(
            image_set="node21_noduleDataset_train_B", args=args
        )  ## NODE21 Dataset - Training B set
        if args.distributed:
            sampler_train_Node21_B = DistributedSampler(dataset_train_loc_Node21_B)
        else:
            sampler_train_Node21_B = torch.utils.data.RandomSampler(
                dataset_train_loc_Node21_B
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_Node21_B, args.batch_size, drop_last=True
        )
        train_loader_loc_Node21_B = DataLoader(
            dataset_train_loc_Node21_B,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        ...
        dataset_train_loc_CANDIDptx = build_dataset(
            image_set="candidptx_pneumothorax_train_A", args=args
        )  ## CANDID-PTX Dataset - Training A set # candidptx_pneumothorax_train_full | candidptx_pneumothorax_train_A | candidptx_pneumothorax_train_B
        dataset_val_loc_CANDIDptx = build_dataset(
            image_set="candidptx_pneumothorax_val", args=args
        )  # candidptx_pneumothorax_val | candidptx_pneumothorax_test
        if args.distributed:
        ...
        dataset_train_loc_CANDIDptx_B = build_dataset(
            image_set="candidptx_pneumothorax_train_B", args=args
        )  ## CANDID-PTX Dataset - Training B set
        if args.distributed:
            sampler_train_CANDIDptx_B = DistributedSampler(
                dataset_train_loc_CANDIDptx_B
            )
        else:
            sampler_train_CANDIDptx_B = torch.utils.data.RandomSampler(
                dataset_train_loc_CANDIDptx_B
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_CANDIDptx_B, args.batch_size, drop_last=True
        )
        train_loader_loc_CANDIDptx_B = DataLoader(
            dataset_train_loc_CANDIDptx_B,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        ...
        ## -- ChestX-Det Dataset -- ##
        dataset_train_loc_ChestXDet = build_dataset(
            image_set="chestxdet_train_A", args=args
        )  ## ChestX-Det Dataset - Training A set # chestxdet_train | chestxdet_train_A | chestxdet_train_B
        ...
        if args.distributed:
            ...
            sampler_train_ChestXDet = torch.utils.data.RandomSampler(
                dataset_train_loc_ChestXDet
            )
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_loc_ChestXDet)
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_ChestXDet, args.batch_size, drop_last=True
        )
        train_loader_loc_ChestXDet = DataLoader(
            dataset_train_loc_ChestXDet,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )  # RETURN
        test_loader_loc_ChestXDet = DataLoader(
            dataset_val_loc_ChestXDet,
            1,
            sampler=sampler_val,
            drop_last=False,
            collate_fn=utils.collate_fn,
        ...
        dataset_train_loc_ChestXDet_B = build_dataset(
            image_set="chestxdet_train_B", args=args
        )  ## ChestX-Det Dataset - Training B set
        if args.distributed:
            sampler_train_ChestXDet_B = DistributedSampler(
                dataset_train_loc_ChestXDet_B
            )
        else:
            sampler_train_ChestXDet_B = torch.utils.data.RandomSampler(
                dataset_train_loc_ChestXDet_B
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_ChestXDet_B, args.batch_size, drop_last=True
        )
        train_loader_loc_ChestXDet_B = DataLoader(
            dataset_train_loc_ChestXDet_B,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        ...
        dataset_train_loc_RSNApneumonia = build_dataset(
            image_set="rsnaPneumoniaDetection_Train_A", args=args
        )  ## RSNApneumonia Dataset - Training A set # rsnaPneumoniaDetection_Train | rsnaPneumoniaDetection_Train_A | rsnaPneumoniaDetection_Train_B
        dataset_val_loc_RSNApneumonia = build_dataset(
            image_set="rsnaPneumoniaDetection_Valid", args=args
        )
        if args.distributed:
            sampler_train_RSNApneumonia = DistributedSampler(
                dataset_train_loc_RSNApneumonia
            )
            sampler_val = DistributedSampler(
                dataset_val_loc_RSNApneumonia, shuffle=False
            )
        else:
            sampler_train_RSNApneumonia = torch.utils.data.RandomSampler(
                dataset_train_loc_RSNApneumonia
            )
            sampler_val = torch.utils.data.SequentialSampler(
                dataset_val_loc_RSNApneumonia
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_RSNApneumonia, args.batch_size, drop_last=True
        )
        train_loader_loc_RSNApneumonia = DataLoader(
            dataset_train_loc_RSNApneumonia,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )  # RETURN
        test_loader_loc_RSNApneumonia = DataLoader(
            dataset_val_loc_RSNApneumonia,
            1,
            sampler=sampler_val,
            drop_last=False,
        ...
        dataset_train_loc_RSNApneumonia_B = build_dataset(
            image_set="rsnaPneumoniaDetection_Train_B", args=args
        )  ## RSNApneumonia Dataset - Training B set
        if args.distributed:
            sampler_train_RSNApneumonia_B = DistributedSampler(
                dataset_train_loc_RSNApneumonia_B
            )
        else:
            sampler_train_RSNApneumonia_B = torch.utils.data.RandomSampler(
                dataset_train_loc_RSNApneumonia_B
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_RSNApneumonia_B, args.batch_size, drop_last=True
        )
        train_loader_loc_RSNApneumonia_B = DataLoader(
            dataset_train_loc_RSNApneumonia_B,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        ...
        ## -- SIIM-ACR Dataset -- ##
        dataset_train_loc_SiimACR = build_dataset(
            image_set="siimacr_train_A", args=args
        )  ## RSNApneumonia Dataset - Training A set # rsnaPneumoniaDetection_Train | rsnaPneumoniaDetection_Train_A | rsnaPneumoniaDetection_Train_B
        ...
        if args.distributed:
            ...
            sampler_train_SiimACR = torch.utils.data.RandomSampler(
                dataset_train_loc_SiimACR
            )
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_loc_SiimACR)
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_SiimACR, args.batch_size, drop_last=True
        )
        train_loader_loc_SiimACR = DataLoader(
            dataset_train_loc_SiimACR,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )  # RETURN
        test_loader_loc_SiimACR = DataLoader(
            dataset_val_loc_SiimACR,
            1,
            sampler=sampler_val,
            drop_last=False,
            collate_fn=utils.collate_fn,
        ...
        dataset_train_loc_SiimACR_B = build_dataset(
            image_set="siimacr_train_B", args=args
        )  ## RSNApneumonia Dataset - Training B set
        if args.distributed:
            sampler_train_SiimACR_B = DistributedSampler(dataset_train_loc_SiimACR_B)
        else:
            sampler_train_SiimACR_B = torch.utils.data.RandomSampler(
                dataset_train_loc_SiimACR_B
            )
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_SiimACR_B, args.batch_size, drop_last=True
        )
        train_loader_loc_SiimACR_B = DataLoader(
            dataset_train_loc_SiimACR_B,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        ...
        train_dataset = chestxdet_dataset(
            image_path="/scratch/jliang12/data/ChestX-Det/train/",
            masks_path="/scratch/jliang12/data/ChestX-Det/train_binary_mask/",
            image_size=(args.imgsize, args.imgsize),
            mode="train",
        )  ## ChestXDetDataset chestxdet_dataset
        train_loader_seg_ChestXDet = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        test_dataset = chestxdet_dataset(
            image_path="/scratch/jliang12/data/ChestX-Det/test/",
            masks_path="/scratch/jliang12/data/ChestX-Det/test_binary_mask/",
            image_size=(args.imgsize, args.imgsize),
            mode="test",
        )
        test_loader_seg_ChestXDet = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        train_dataset = SIIM_PXSDataset(
            [
                (
                    "/scratch/jliang12/data/siim_pneumothorax_segmentation/train_jpeg",
                    "data/pxs/train.txt",
                )
            ],
            image_size=(args.imgsize, args.imgsize),
        ...
        train_loader_seg_SIIM = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        test_dataset = SIIM_PXSDataset(
            [
                (
                    "/scratch/jliang12/data/siim_pneumothorax_segmentation/test_jpeg",
                    "data/pxs/test.txt",
                )
            ],
            image_size=(args.imgsize, args.imgsize),
        ...
        test_loader_seg_SIIM = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
        ...
        train_dataset = Candid_PTX_PXSDataset(
            [
                (
                    "/scratch/jliang12/data/CANDID-PTX/dataset",
                    "data/candid_ptx/train.txt",
                )
            ],
            image_size=(args.imgsize, args.imgsize),
            mode="train",
        ...
        test_dataset = Candid_PTX_PXSDataset(
            [("/scratch/jliang12/data/CANDID-PTX/dataset", "data/candid_ptx/test.txt")],
            image_size=(args.imgsize, args.imgsize),
            mode="test",
        )
        sampler = torch.utils.data.RandomSampler(test_dataset)
        ...
        return (
            train_loader_cls_TBX11k,
            test_loader_cls_TBX11k,
            train_loader_cls_NODE21,
            test_loader_cls_NODE21,
            train_loader_cls_ChestXDet,
            test_loader_cls_ChestXDet,
            train_loader_cls_RSNApneumonia,
            test_loader_cls_RSNApneumonia,
            train_loader_cls_SIIMACRptx,
            test_loader_cls_SIIMACRptx,
            train_loader_cls_CANDIDptx,
            test_loader_cls_CANDIDptx,
            train_loader_loc_TBX11k,
            test_loader_loc_TBX11k,
            dataset_val_loc_TBX11k,
            sampler_train_TBX11k,
            train_loader_loc_TBX11k_B,
            train_loader_loc_Node21,
            test_loader_loc_Node21,
            dataset_val_loc_Node21,
            sampler_train_Node21,
            train_loader_loc_Node21_B,
            train_loader_loc_CANDIDptx,
            test_loader_loc_CANDIDptx,
            dataset_val_loc_CANDIDptx,
            sampler_train_CANDIDptx,
            train_loader_loc_CANDIDptx_B,
            train_loader_loc_ChestXDet,
            test_loader_loc_ChestXDet,
            dataset_val_loc_ChestXDet,
            sampler_train_ChestXDet,
            train_loader_loc_ChestXDet_B,
            train_loader_loc_RSNApneumonia,
            test_loader_loc_RSNApneumonia,
            dataset_val_loc_RSNApneumonia,
            sampler_train_RSNApneumonia,
            train_loader_loc_RSNApneumonia_B,
            train_loader_loc_SiimACR,
            test_loader_loc_SiimACR,
            dataset_val_loc_SiimACR,
            sampler_train_SiimACR,
            train_loader_loc_SiimACR_B,
            train_loader_seg_ChestXDet,
            test_loader_seg_ChestXDet,
            train_loader_seg_SIIM,
    ...
    if args.taskcomponent in ["foundation_x2_pretraining"]:
        ...
        ## -- TBX11K Dataset -- ##
        dataset_train_loc_TBX11k = build_dataset(
            image_set="tbx11k_catagnostic_train", args=args
        )  ## TBX11K Dataset - Training A set # tbx11k_catagnostic_train | tbx11k_catagnostic_train_A | tbx11k_catagnostic_train_B
        dataset_val_loc_TBX11k = build_dataset(
            image_set="tbx11k_catagnostic_test", args=args
        )
        if args.distributed:
            sampler_train_TBX11k = DistributedSampler(
                dataset_train_loc_TBX11k, shuffle=True
            )
            sampler_val = DistributedSampler(dataset_val_loc_TBX11k, shuffle=False)
            # sampler_test = DistributedSampler(dataset_test, shuffle=False) # added by Nahid
        else:
        ...
        ## -- NODE21 Dataset -- ##
        dataset_train_loc_Node21 = build_dataset(
            image_set="node21_noduleDataset_train", args=args
        )  ## NODE21 Dataset - Training A set # node21_noduleDataset_train | node21_noduleDataset_train_A | node21_noduleDataset_train_B
        dataset_val_loc_Node21 = build_dataset(
            image_set="node21_noduleDataset_test", args=args
        )
        if args.distributed:
            sampler_train_Node21 = DistributedSampler(
                dataset_train_loc_Node21, shuffle=True
            )
            sampler_val = DistributedSampler(dataset_val_loc_Node21, shuffle=False)
        else:
            sampler_train_Node21 = torch.utils.data.RandomSampler(
        ...
        ## -- CANDID-PTX Dataset -- ##
        dataset_train_loc_CANDIDptx = build_dataset(
            image_set="candidptx_pneumothorax_train_full", args=args
        )  ## CANDID-PTX Dataset - Training A set # candidptx_pneumothorax_train_full | candidptx_pneumothorax_train_A | candidptx_pneumothorax_train_B
        dataset_val_loc_CANDIDptx = build_dataset(
            image_set="candidptx_pneumothorax_val", args=args
        )  # candidptx_pneumothorax_val | candidptx_pneumothorax_test
        ...
        ## -- ChestX-Det Dataset -- ##
        dataset_train_loc_ChestXDet = build_dataset(
            image_set="chestxdet_train", args=args
        )  ## ChestX-Det Dataset - Training A set # chestxdet_train | chestxdet_train_A | chestxdet_train_B
        dataset_val_loc_ChestXDet = build_dataset(image_set="chestxdet_test", args=args)
        if args.distributed:
            sampler_train_ChestXDet = DistributedSampler(
                dataset_train_loc_ChestXDet, shuffle=True
            )
            sampler_val = DistributedSampler(dataset_val_loc_ChestXDet, shuffle=False)
        else:
            sampler_train_ChestXDet = torch.utils.data.RandomSampler(
        ...
        ## -- RSNA Pneumonia Challenge Dataset -- ##
        dataset_train_loc_RSNApneumonia = build_dataset(
            image_set="rsnaPneumoniaDetection_Train", args=args
        )  ## RSNApneumonia Dataset - Training A set # rsnaPneumoniaDetection_Train | rsnaPneumoniaDetection_Train_A | rsnaPneumoniaDetection_Train_B
        dataset_val_loc_RSNApneumonia = build_dataset(
            image_set="rsnaPneumoniaDetection_Valid", args=args
        )
        if args.distributed:
            sampler_train_RSNApneumonia = DistributedSampler(
                dataset_train_loc_RSNApneumonia, shuffle=True
            )
            sampler_val = DistributedSampler(
                dataset_val_loc_RSNApneumonia, shuffle=False
            )
        ...
        ## -- SIIM-ACR Dataset -- ##
        dataset_train_loc_SiimACR = build_dataset(
            image_set="siimacr_train", args=args
        )  ## RSNApneumonia Dataset - Training A set # siimacr_train | siimacr_train_A | siimacr_train_B
        dataset_val_loc_SiimACR = build_dataset(image_set="siimacr_val", args=args)
        if args.distributed:
            sampler_train_SiimACR = DistributedSampler(
                dataset_train_loc_SiimACR, shuffle=True
            )
            sampler_val = DistributedSampler(dataset_val_loc_SiimACR, shuffle=False)
        else:
            sampler_train_SiimACR = torch.utils.data.RandomSampler(
    ...
    if args.taskcomponent in [
        ...
        train_loader_cls_NIHChestXray14 = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_NIHChestXray14 = DataLoader(
            dataset=dataset_test,
        ...
        data_dir = _dataset_location(
            ...
            "chexpert",
            "images",
            default=DATASETS_CONFIG["cls_chexpert_root"],
        )
        dataset_train = CheXpert(
            images_path=data_dir,
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", crop_size=args.imgsize, mode="train"
        ...
        train_loader_cls_CheXpert = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_CheXpert = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size // 2,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        data_dir = _dataset_location(
            ...
            "vindr_cxr",
            "images",
            default=DATASETS_CONFIG["cls_vindrcxr_root"],
        )
        dataset_train = VinDrCXR(
            images_path=data_dir,
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", crop_size=args.imgsize, mode="train"
        ...
        train_loader_cls_VinDRCXR = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_VinDRCXR = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size // 2,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        data_dir = _dataset_location(
            ...
            "shenzhen",
            "images",
            default=DATASETS_CONFIG["cls_shenzencxr_root"],
        )
        dataset_train = ShenzhenCXR(
            images_path=data_dir,
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", crop_size=args.imgsize, mode="train"
        ...
        train_loader_cls_NIHShenzhen = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_NIHShenzhen = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size // 2,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        data_dir = _dataset_location(
            ...
            "mimic_cxr",
            "images",
            default=DATASETS_CONFIG["cls_mimiccxr_root"],
        )
        dataset_train = MIMIC(
            images_path=data_dir,
            file_path=train_list,
        ...
        train_loader_cls_MIMICII = DataLoader(
            dataset=dataset_train,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        ...
        test_loader_cls_MIMICII = DataLoader(
            dataset=dataset_test,
            batch_size=args.batch_size // 2,
            shuffle=False,
            num_workers=args.num_workers,
        ...
        data_dir = _dataset_location(
            ...
            "tbx11k",
            "images",
            default=DATASETS_CONFIG["cls_tbx11k_root"],
        )
        dataset_train = TBX11KDataset(
            images_path=data_dir,
            file_path=train_list,
        ...
        data_dir = _dataset_location(
            ...
            "node21",
            "images",
            default=DATASETS_CONFIG["cls_node21_root"],
        )
        dataset_train = NODE21(
            images_path=data_dir,
            file_path=train_list,
        ...
        data_dir = _dataset_location(
            ...
            "chestx_det",
            "images",
            default=DATASETS_CONFIG["cls_chestxdet_root"],
        )
        train_list = _dataset_location(
            "classification",
            "chestx_det",
            "splits",
            "train_json",
            default=DATASETS_CONFIG["cls_chestxdet_trainList"],
        )
        test_list = _dataset_location(
            "classification",
            "chestx_det",
            "splits",
            "test_json",
            default=DATASETS_CONFIG["cls_chestxdet_testList"],
        )
        dataset_train = ChestXDet_cls(
            images_path=data_dir,
            file_path="train",
        ...
        dataset_train = RSNAPneumonia(
            images_path=DATASETS_CONFIG["cls_rsnapneumonia_root"],
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", crop_size=args.imgsize, mode="train"
            ),
        ...
        dataset_train = SIIMPTX(
            images_path=DATASETS_CONFIG["cls_siimacr_root"],
            file_path=train_list,
            augment=build_transform_classification(
                normalize="chestx-ray", crop_size=args.imgsize, mode="train"
            ),
        ...
        ## -- TBX11K Dataset -- ##
        dataset_train_loc_TBX11k = build_dataset(
            image_set=DATASETS_CONFIG["loc_tbx11k_trainTag"], args=args
        )  ## TBX11K Dataset - Training A set # tbx11k_catagnostic_train | tbx11k_catagnostic_train_A | tbx11k_catagnostic_train_B
        dataset_val_loc_TBX11k = build_dataset(
            image_set=DATASETS_CONFIG["loc_tbx11k_testTag"], args=args
        )
        if args.distributed:
            sampler_train_TBX11k = DistributedSampler(
                dataset_train_loc_TBX11k, shuffle=True
        ...
        ## -- NODE21 Dataset -- ##
        dataset_train_loc_Node21 = build_dataset(
            image_set=DATASETS_CONFIG["loc_node21_trainTag"], args=args
        )  ## NODE21 Dataset - Training A set # node21_noduleDataset_train | node21_noduleDataset_train_A | node21_noduleDataset_train_B
        dataset_val_loc_Node21 = build_dataset(
            image_set=DATASETS_CONFIG["loc_node21_testTag"], args=args
        )
        if args.distributed:
            sampler_train_Node21 = DistributedSampler(
                dataset_train_loc_Node21, shuffle=True
        ...
        ## -- ChestX-Det Dataset -- ##
        dataset_train_loc_ChestXDet = build_dataset(
            image_set=DATASETS_CONFIG["loc_chestxdet_trainTag"], args=args
        )  ## ChestX-Det Dataset - Training A set # chestxdet_train | chestxdet_train_A | chestxdet_train_B
        dataset_val_loc_ChestXDet = build_dataset(
            image_set=DATASETS_CONFIG["loc_chestxdet_testTag"], args=args
        )
        if args.distributed:
            sampler_train_ChestXDet = DistributedSampler(
                dataset_train_loc_ChestXDet, shuffle=True
        ...
        ## -- RSNA Pneumonia Challenge Dataset -- ##
        dataset_train_loc_RSNApneumonia = build_dataset(
            image_set=DATASETS_CONFIG["loc_rsnapneumonia_trainTag"], args=args
        )  ## RSNApneumonia Dataset - Training A set # rsnaPneumoniaDetection_Train | rsnaPneumoniaDetection_Train_A | rsnaPneumoniaDetection_Train_B
        dataset_val_loc_RSNApneumonia = build_dataset(
            image_set=DATASETS_CONFIG["loc_rsnapneumonia_testTag"], args=args
        )
        if args.distributed:
            sampler_train_RSNApneumonia = DistributedSampler(
                dataset_train_loc_RSNApneumonia, shuffle=True
        ...
        ## -- SIIM-ACR Dataset -- ##
        dataset_train_loc_SiimACR = build_dataset(
            image_set=DATASETS_CONFIG["loc_siimacr_trainTag"], args=args
        )  ## RSNApneumonia Dataset - Training A set # siimacr_train | siimacr_train_A | siimacr_train_B
        dataset_val_loc_SiimACR = build_dataset(
            image_set=DATASETS_CONFIG["loc_siimacr_testTag"], args=args
        )
        if args.distributed:
            sampler_train_SiimACR = DistributedSampler(
                dataset_train_loc_SiimACR, shuffle=True
        ...
        train_dataset = chestxdet_dataset(
            image_path=DATASETS_CONFIG["seg_chestxdet_trainRoot"],
            masks_path=DATASETS_CONFIG["seg_chestxdet_trainList"],
            image_size=(args.imgsize, args.imgsize),
            mode="train",
        )  ## ChestXDetDataset chestxdet_dataset
        train_loader_seg_ChestXDet = torch.utils.data.DataLoader(
        ...
        test_dataset = chestxdet_dataset(
            image_path=DATASETS_CONFIG["seg_chestxdet_testRoot"],
            masks_path=DATASETS_CONFIG["seg_chestxdet_testList"],
            image_size=(args.imgsize, args.imgsize),
            mode="test",
        )
        test_loader_seg_ChestXDet = torch.utils.data.DataLoader(
        ...
        train_dataset = SIIM_PXSDataset(
            [
                (
                    DATASETS_CONFIG["seg_siimacr_trainRoot"],
                    DATASETS_CONFIG["seg_siimacr_trainList"],
                )
            ],
            image_size=(args.imgsize, args.imgsize),
            mode="train",
        ...
        test_dataset = SIIM_PXSDataset(
            [
                (
                    DATASETS_CONFIG["seg_siimacr_testRoot"],
                    DATASETS_CONFIG["seg_siimacr_testList"],
                )
            ],
            image_size=(args.imgsize, args.imgsize),
            mode="test",
        ...
        return (
            train_loader_cls_CheXpert,
            test_loader_cls_CheXpert,
            train_loader_cls_NIHChestXray14,
            test_loader_cls_NIHChestXray14,
            train_loader_cls_VinDRCXR,
            test_loader_cls_VinDRCXR,
            train_loader_cls_NIHShenzhen,
            test_loader_cls_NIHShenzhen,
            train_loader_cls_MIMICII,
            test_loader_cls_MIMICII,
            train_loader_cls_TBX11k,
            test_loader_cls_TBX11k,
            train_loader_cls_NODE21,
            test_loader_cls_NODE21,
    ...
    #     test_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/ark6_classificationTask/CheXpert_test_official.csv'
    #     if args.serverC == 'DFS':
    #         data_dir = "/mnt/dfs/nuislam/Data/ChestXRay14_images/" #"/mnt/dfs/nuislam/Data/ChestXRay14_images/" ## "/data/jliang12/jpang12/dataset/nih_xray14/images/images/"
    #     elif args.serverC == "SOL":
    ...
    #     elif args.serverC == "SOL":
    #         data_dir = "/scratch/jliang12/data/ChestX-Det/"
    #         train_list = '/scratch/jliang12/FoundationX_localization_bbox_annotation_collections/ChestX-Det_Disease/ChestX_det_train_NAD_v2.json'
    #         test_list = '/scratch/jliang12/FoundationX_localization_bbox_annotation_collections/ChestX-Det_Disease/ChestX_det_test_NAD_v2.json'
    ...
    #     valid_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/rsna_pneumonia/RSNAPneumonia_val.txt'
    #     test_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/rsna_pneumonia/RSNAPneumonia_test.txt'
    #     dataset_train = RSNAPneumonia(images_path="/scratch/jliang12/data/rsna-pneumonia-detection-challenge/stage_2_train_images_png/", file_path=train_list, augment=build_transform_classification(normalize="chestx-ray", crop_size=args.imgsize, mode="train"))
    #     dataset_test = RSNAPneumonia(images_path="/scratch/jliang12/data/rsna-pneumonia-detection-challenge/stage_2_train_images_png/", file_path=test_list, augment=build_transform_classification(normalize="chestx-ray", crop_size=args.imgsize, mode="test2"))
    ...
    #     valid_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/siimacr_ptx/SIIMPTX_cls_val.txt'
    #     test_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/siimacr_ptx/SIIMPTX_cls_test.txt'
    #     dataset_train = SIIMPTX(images_path="/scratch/jliang12/data/siim_pneumothorax_segmentation/", file_path=train_list, augment=build_transform_classification(normalize="chestx-ray", crop_size=args.imgsize, mode="train"))
    #     dataset_test = SIIMPTX(images_path="/scratch/jliang12/data/siim_pneumothorax_segmentation/", file_path=test_list, augment=build_transform_classification(normalize="chestx-ray", crop_size=args.imgsize, mode="test2"))
    ...
    #     valid_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/candid_ptx/CANDIDPTX_cls_val.txt'
    #     test_list = '/scratch/nuislam/Model_Checkpoints/data_files_splits/candid_ptx/CANDIDPTX_cls_test.txt'
    #     dataset_train = CANDIDPTX(images_path="/scratch/jliang12/data/CANDID-PTX/dataset/", file_path=train_list, augment=build_transform_classification(normalize="chestx-ray", crop_size=args.imgsize, mode="train"))
    #     dataset_test = CANDIDPTX(images_path="/scratch/jliang12/data/CANDID-PTX/dataset/", file_path=test_list, augment=build_transform_classification(normalize="chestx-ray", crop_size=args.imgsize, mode="test2"))
    ...
    if args.taskcomponent in ["photographic_datasets"]:
        ## TaskHead_number  0 = COCO Localization
        ## TaskHead_number  1 = Objects365 Localization
        ## TaskHead_number  2 = WIDER FACE Localization

        ## Dataloaders for Localization -------------------------------------------------------------

        ## -- COCO Dataset -- ##
        dataset_train_coco = build_dataset(
            image_set="train", args=args
        )  ## COCO dataset
        dataset_val_coco = build_dataset(image_set="val", args=args)
        if args.distributed:
            sampler_train_coco = DistributedSampler(dataset_train_coco)
            sampler_val = DistributedSampler(dataset_val_coco, shuffle=False)
            # sampler_test = DistributedSampler(dataset_test, shuffle=False) # added by Nahid
        else:
            sampler_train_coco = torch.utils.data.RandomSampler(dataset_train_coco)
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_coco)
            # sampler_test = torch.utils.data.SequentialSampler(dataset_test) # added by Nahid
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_coco, args.batch_size, drop_last=True
        )
        ...
        ## -- Objects365 Dataset -- ##
        dataset_train_objects365 = build_dataset(
            image_set="objects365_train", args=args
        )
        dataset_test_objects365 = build_dataset(image_set="objects365_test", args=args)
        if args.distributed:
            sampler_train_objects365 = DistributedSampler(dataset_train_objects365)
            sampler_val = DistributedSampler(dataset_test_objects365, shuffle=False)
        else:
            sampler_train_objects365 = torch.utils.data.RandomSampler(
                dataset_train_objects365
            )
            sampler_val = torch.utils.data.SequentialSampler(dataset_test_objects365)
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train_objects365, args.batch_size, drop_last=True
        )
        train_loader_objects365 = DataLoader(
            dataset_train_objects365,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        test_loader_objects365 = DataLoader(
            dataset_test_objects365,
            1,
            sampler=sampler_val,
            drop_last=False,
            collate_fn=utils.collate_fn,
        ...
        # -- WIDER FACE -- ##
        dataset_train_widerface = build_dataset(image_set="widerface_train", args=args)
        dataset_val_widerface = build_dataset(image_set="widerface_val", args=args)
        if args.distributed:
            sampler_train_widerface = DistributedSampler(dataset_train_widerface)
            sampler_val = DistributedSampler(dataset_val_widerface, shuffle=False)
        else:
            sampler_train_widerface = torch.utils.data.RandomSampler(
                dataset_train_widerface
            )
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_widerface)
        # WIDER FACE batch size cannot be above 6
        if args.batch_size > 6:
            batch_sampler_train = torch.utils.data.BatchSampler(
                sampler_train_widerface, 6, drop_last=True
            )
        else:
            batch_sampler_train = torch.utils.data.BatchSampler(
                sampler_train_widerface, args.batch_size, drop_last=True
            )
        train_loader_widerface = DataLoader(
            dataset_train_widerface,
            batch_sampler=batch_sampler_train,
            collate_fn=utils.collate_fn,
            num_workers=args.num_workers,
        )
        test_loader_widerface = DataLoader(
            dataset_val_widerface,
            1,
            sampler=sampler_val,
            drop_last=False,
            collate_fn=utils.collate_fn,
        ...
        return (
            ...
            dataset_val_coco,
            sampler_train_coco,
            train_loader_objects365,
            test_loader_objects365,
            dataset_test_objects365,
            sampler_train_objects365,
            train_loader_widerface,
            test_loader_widerface,
            dataset_val_widerface,
            sampler_train_widerface,
        )

    if args.taskcomponent in ["benchmark"]:
        # Taskhead_number 0 = ImageNet classification - ready
        # Taskhead_number 3 = CompCars classification - not ready
        # Taskhead_number 4 = Tsinghua Dogs classification - not ready

        # Taskhead_number 9 = COCO-Stuff segmentation - ready

        ## Dataloaders for CLASSIFICATION ----------------------------------------------------------------------------

        ## --- ImageNet Classification --- ##
        if args.dataset_file == "imagenet":
            train_dir = "/scratch/jliang12/data/ImageNet/train"
            val_dir = "/scratch/jliang12/data/ImageNet/val"
            ...
            # print(f"Batch {i} target labels: {target}")

            return train_loader, test_loader

        ## --- CompCars Classification --- ##
        if args.dataset_file == "compcars":
            img_dir = "/ocean/projects/med230002p/nalberti/data/CompCars/sv_data/image"
            train_list = "/ocean/projects/med230002p/nalberti/data/CompCars/sv_data/train_surveillance.txt"
            val_list = "/ocean/projects/med230002p/nalberti/data/CompCars/sv_data/test_surveillance.txt"
        ...
        ## --- Tsinghua Dogs Classification --- ##
        if args.dataset_file == "tsinghuadogs":
            img_dir = (
                "/ocean/projects/med230002p/nalberti/data/Tsinghua_Dogs/low-resolution"
            )
            train_lst = "/ocean/projects/med230002p/nalberti/data/Tsinghua_Dogs/TrainAndValList/train.lst"
            val_lst = "/ocean/projects/med230002p/nalberti/data/Tsinghua_Dogs/TrainAndValList/val.lst"
            ...
            test_dataset = TsinghuaDogsDataset(
                img_dir,
                train_lst,
                transform=build_transform_classification(
                    normalize="imagenet", mode="test2"
        ...
        if args.dataset_file == "coco-stuff":
            cocostuff_train_imgs = (
                "/scratch/nralbert/research/data/coco-stuff/train2017"
            )
            cocostuff_val_imgs = "/scratch/nralbert/research/data/coco-stuff/val2017"
            # cocostuff_train_ann = "/scratch/nralbert/research/data/coco-stuff/annotations/instances_train2017.json"
            # cocostuff_val_ann = "/scratch/nralbert/research/data/coco-stuff/annotations/instances_val2017.json"

            train_dataset = COCOSegmentationDataset(
                root_dir=cocostuff_train_imgs,
                annotation_dir=cocostuff_train_imgs,
                mode="train",
            )
            sampler = torch.utils.data.RandomSampler(train_dataset)
            train_loader = torch.utils.data.DataLoader(
            ...
            test_dataset = COCOSegmentationDataset(
                root_dir=cocostuff_val_imgs,
                annotation_dir=cocostuff_val_imgs,
                mode="test",
            )
    ...
    if (
        args.taskcomponent
        in [
            "foundation_x3_FineTuning",
            "foundation_x4_pretraining",
            "foundation_x5_pretraining",
        ]
        and args.cyclictask == "segFT_TESTseg_candidptxSEG"
    ):
        train_dataset = Candid_PTX_PXSDataset(
            [
                (
    ...
    if (
        ...
        and args.cyclictask == "segFT_TESTseg_chestxdetSEG"
    ):
        train_dataset = chestxdet_dataset(
            image_path="/scratch/jliang12/data/ChestX-Det/train/",
            masks_path="/scratch/jliang12/data/ChestX-Det/train_binary_mask/",
    ...
    if (
        ...
        and args.cyclictask == "segFT_TESTseg_siimacrSEG"
    ):
        train_dataset = SIIM_PXSDataset(
            [
                (
        ...
        pred_file = os.path.join(
            "/scratch/nuislam/Model_Checkpoints/IntegratedModel_DINOpipeline/IntegratedModel_FoundationX3/TEST_SIIM-ACR/",
            "prediction",
        )
        device = torch.device("cuda")
        # Original SOL path: /data/jliang12/jpang12/dataset/Pneumothorax_segmentation/stage_2_images/
        ptx_stage2_images = _dataset_location(
            "directories",
            "ptx_stage2_images",
            default="/scratch/sejong/class-dataset/Pneumothorax_segmentation/stage_2_images/",
        )
        dataset_test = SIIMPneumothorax_test_original(
            images_path=ptx_stage2_images,
            image_file="stage_2_sample_submission.csv",
            image_size=(224, 224),
        )
        test_loader_seg_SIIM = torch.utils.data.DataLoader(
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "locFT_TESTloc_tbx11kLOC"
    ):
        dataset_train_loc_TBX11k = build_dataset(
            image_set="tbx11k_catagnostic_train", args=args
        )  ## TBX11K Dataset - Training A set # tbx11k_catagnostic_train | tbx11k_catagnostic_train_A | tbx11k_catagnostic_train_B
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "locFT_TESTloc_node21LOC"
    ):
        dataset_train_loc_Node21 = build_dataset(
            image_set="node21_noduleDataset_train", args=args
        )  ## NODE21 Dataset - Training A set # node21_noduleDataset_train | node21_noduleDataset_train_A | node21_noduleDataset_train_B
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "locFT_TESTloc_candidptxLOC"
    ):
        dataset_train_loc_CANDIDptx = build_dataset(
            image_set="candidptx_pneumothorax_train_full", args=args
        )  ## CANDID-PTX Dataset - Training A set # candidptx_pneumothorax_train_full | candidptx_pneumothorax_train_A | candidptx_pneumothorax_train_B
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "locFT_TESTloc_rsnapneumoniaLOC"
    ):
        dataset_train_loc_RSNApneumonia = build_dataset(
            image_set="rsnaPneumoniaDetection_Train", args=args
        )  ## RSNApneumonia Dataset - Training A set # rsnaPneumoniaDetection_Train | rsnaPneumoniaDetection_Train_A | rsnaPneumoniaDetection_Train_B
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "locFT_TESTloc_chestxdetLOC"
    ):
        ## -- ChestX-Det Dataset -- ##
        dataset_train_loc_ChestXDet = build_dataset(
            image_set="chestxdet_train", args=args
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "locFT_TESTloc_siimacrLOC"
    ):
        dataset_train_loc_SiimACR = build_dataset(
            image_set="siimacr_train", args=args
        )  ## RSNApneumonia Dataset - Training A set # siimacr_train | siimacr_train_A | siimacr_train_B
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_nihchestxray14CLS"
    ):
        train_list = "data/xray14/official/train_official.txt"
        val_list = "data/xray14/official/val_official.txt"
        test_list = "data/xray14/official/test_official.txt"
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_chexpertCLS"
    ):
        train_list = _dataset_location(
            "classification",
            "chexpert",
            "splits",
            "train",
            default=DATASETS_CONFIG["cls_chexpert_trainList"],
        )
        val_list = _dataset_location(
            "classification",
            "chexpert",
            "splits",
            "val",
            default=DATASETS_CONFIG["cls_chexpert_valList"],
        )
        test_list = _dataset_location(
            "classification",
            "chexpert",
            "splits",
            "test",
            default=DATASETS_CONFIG["cls_chexpert_testList"],
        )
        # Original SOL path: /data/jliang12/jpang12/dataset/CheXpert-v1.0/
        data_dir = _dataset_location(
            "directories",
            "chexpert_v1_root",
            default=DATASETS_CONFIG["cls_chexpert_root"],
        ...
        return train_loader_cls_CheXpert, test_loader_cls_CheXpert, 0, 0

    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_vindrcxrCLS"
    ):
        train_list = _dataset_location(
            "classification",
            "vindr_cxr",
            "splits",
            "train",
            default=DATASETS_CONFIG["cls_vindrcxr_trainList"],
        )
        val_list = _dataset_location(
            "classification",
            "vindr_cxr",
            "splits",
            "val",
            default=DATASETS_CONFIG["cls_vindrcxr_valList"],
        )
        test_list = _dataset_location(
            "classification",
            "vindr_cxr",
            "splits",
            "test",
            default=DATASETS_CONFIG["cls_vindrcxr_testList"],
        )
        data_dir = _dataset_location(
            "classification",
            "vindr_cxr",
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_nihshenzenCLS"
    ):
        train_list = _dataset_location(
            "classification",
            "shenzhen",
            "splits",
            "train",
            default=DATASETS_CONFIG["cls_shenzencxr_trainList"],
        )
        val_list = _dataset_location(
            "classification",
            "shenzhen",
            "splits",
            "val",
            default=DATASETS_CONFIG["cls_shenzencxr_valList"],
        )
        test_list = _dataset_location(
            "classification",
            "shenzhen",
            "splits",
            "test",
            default=DATASETS_CONFIG["cls_shenzencxr_testList"],
        )
        data_dir = _dataset_location(
            "classification",
            "shenzhen",
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_mimic2CLS"
    ):
        train_list = _dataset_location(
            "classification",
            "mimic_cxr",
            "splits",
            "train",
            default=DATASETS_CONFIG["cls_mimiccxr_trainList"],
        )
        val_list = _dataset_location(
            "classification",
            "mimic_cxr",
            "splits",
            "val",
            default=DATASETS_CONFIG["cls_mimiccxr_valList"],
        )
        test_list = _dataset_location(
            "classification",
            "mimic_cxr",
            "splits",
            "test",
            default=DATASETS_CONFIG["cls_mimiccxr_testList"],
        )
        data_dir = _dataset_location(
            "classification",
            "mimic_cxr",
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_tbx11kCLS"
    ):
        ## -- TBX11K Dataset -- ##
        train_list = "lists/TBX11K_train.txt"
        test_list = "lists/TBX11K_val.txt"
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_node21CLS"
    ):
        ## -- NODE21 Dataset -- ##
        train_list = "data/node21_dataset/train.txt"
        test_list = "data/node21_dataset/test.txt"
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_chestxdetCLS"
    ):
        ## -- ChestX-Det Dataset -- ##
        data_dir = _dataset_location(
            "classification",
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_rsnapneumoniaCLS"
    ):
        ## -- RSNAPneumonia Dataset -- ##
        train_list = "/scratch/nuislam/Model_Checkpoints/data_files_splits/rsna_pneumonia/RSNAPneumonia_train.txt"
        valid_list = "/scratch/nuislam/Model_Checkpoints/data_files_splits/rsna_pneumonia/RSNAPneumonia_val.txt"
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_siimacrCLS"
    ):
        ## -- SIIMPTX Dataset -- ##
        train_list = "/scratch/nuislam/Model_Checkpoints/data_files_splits/siimacr_ptx/SIIMPTX_cls_train.txt"
        valid_list = "/scratch/nuislam/Model_Checkpoints/data_files_splits/siimacr_ptx/SIIMPTX_cls_val.txt"
    ...
    if (
        args.taskcomponent in ["foundation_x3_FineTuning"]
        and args.cyclictask == "clsFT_TESTcls_candidptxCLS"
    ):
        ## -- CANDID-PTX Dataset -- ##
        train_list = "/scratch/nuislam/Model_Checkpoints/data_files_splits/candid_ptx/CANDIDPTX_cls_train.txt"
        valid_list = "/scratch/nuislam/Model_Checkpoints/data_files_splits/candid_ptx/CANDIDPTX_cls_val.txt"
    ...
    if (
        args.taskcomponent in ["foundation_x_anyDataset"]
        and args.cyclictask == "locFT_TESTloc_vindrLOC_kaggle14disease"
    ):
        dataset_train_loc = build_dataset(
            image_set="vindrcxr_train", args=args
        )  ## ChestX-Det Dataset - Training A set # chestxdet_train | chestxdet_train_A | chestxdet_train_B
        dataset_val_loc = build_dataset(image_set="vindrcxr_test", args=args)
        if args.distributed:
            sampler_train = DistributedSampler(dataset_train_loc, shuffle=True)
            sampler_val = DistributedSampler(dataset_val_loc, shuffle=False)
        else:
            sampler_train = torch.utils.data.RandomSampler(dataset_train_loc)
            sampler_val = torch.utils.data.SequentialSampler(dataset_val_loc)
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train, args.batch_size, drop_last=True
        )
        ...
        return train_loader_loc, test_loader_loc, dataset_val_loc, 0
    ...
    if (
        ...
        dataset_val_loc = build_dataset(image_set="chestxdet_test", args=args)
        if args.distributed:
            sampler_train = DistributedSampler(dataset_train_loc, shuffle=True)
            sampler_val = DistributedSampler(dataset_val_loc, shuffle=False)
        else:
    ...
    if (
        args.taskcomponent in ["foundation_x_anyDataset"]
        and args.cyclictask == "locFT_TESTloc_nihLOC"
    ):
        dataset_train_loc = build_dataset(image_set="nihLOC_test", args=args)
        dataset_val_loc = build_dataset(image_set="nihLOC_test", args=args)
        if args.distributed:
            sampler_train = DistributedSampler(dataset_train_loc, shuffle=True)
            sampler_val = DistributedSampler(dataset_val_loc, shuffle=False)
        else:
