...
from albumentations import Compose, HorizontalFlip, Normalize, VerticalFlip, Rotate, Resize, ShiftScaleRotate, OneOf, GridDistortion, OpticalDistortion, \
    ElasticTransform, GaussNoise, MedianBlur,  Blur, CoarseDropout,RandomBrightnessContrast,RandomGamma, RandomSizedCrop, RandomResizedCrop, ToFloat
...
def _resolve_image_path(path: str) -> str:
    ...
    if image_path.suffix:
        ...
        if image_path.parent.exists():
            for candidate in image_path.parent.glob(f"{base_path.name}.*"):
...
class chestxdet_dataset(Dataset):  # only Segmentation ## From Anni
    ...
    def combine_mask(self,img,masks_list):
        combined_mask = np.zeros( (self.image_size[0],self.image_size[1],len(masks_list)), dtype = np.uint8 )
...
class VinDrCXR(Dataset):
  ...
  def __getitem__(self, index):
    ...
    imagePath = self.img_list[index]
    imageLabel = torch.FloatTensor(self.img_label[index])
    imageData = Image.open(_resolve_image_path(imagePath)).convert('RGB').resize((224,224))
...
class CheXpert(Dataset):
  def __init__(self, images_path, file_path, augment, num_class=14,
               uncertain_label="LSR-Ones", unknown_label=0, annotation_percent=100, mode='train'):
    self.img_list = []
    self.img_label = []
    self.augment = augment
    self.train_augment = build_ts_transformations()
...
class COCOSegmentationDataset(Dataset):
    def __init__(self, root_dir, annotation_dir, image_size=(224,224), mode="train"):
        ...
        self.mask_files = sorted([f for f in os.listdir(annotation_dir) if f.endswith('.png')])
    ...
    def __getitem__(self, idx):
        ...
        imageData = cv2.resize(cv2.imread(img_name,cv2.IMREAD_COLOR),self.image_size, interpolation=cv2.INTER_AREA)
        ...
        leftMaskData = cv2.resize(cv2.imread(mask_name,cv2.IMREAD_GRAYSCALE),self.image_size, interpolation=cv2.INTER_NEAREST)
        # rightMaskData = cv2.resize(cv2.imread(maskPath[1],cv2.IMREAD_GRAYSCALE),self.image_size, interpolation=cv2.INTER_AREA)
...
def dataloader_return(args):
    ...
    if args.taskcomponent == 'segmentation' or args.taskcomponent == 'segmentation_cyclic' or args.taskcomponent == "segmentation_vindrcxr_organ_1h3ch":
        ...
        if args.segmentation_dataset == 'jsrt_lung':
            ...
            jsrt_root = _dataset_location(
                "directories", "jsrt_all247images", default="/scratch/sejong/class-dataset/JSRT/All247images"
            )
    ...
    if args.taskcomponent == "detect_segmentation_cyclic_v2" or args.taskcomponent == "detect_segmentation_cyclic_v3":  ## with EMA ## Training Strategy 1 OR Training Strategy 2
        ...
        data_loader_valHeart = DataLoader(dataset_valHeart, args.batch_size, sampler=sampler_valHeart, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
        ...
        data_loader_valLeftLung = DataLoader(dataset_valLeftLung, args.batch_size, sampler=sampler_valLeftLung, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
        ...
        data_loader_valRightLung = DataLoader(dataset_valRightLung, args.batch_size, sampler=sampler_valRightLung, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
    ...
    if args.taskcomponent == 'classification':
        ...
        if args.classification_dataset == "ChestXray14":
            ...
            data_dir = _dataset_location(
                "directories", "nih_chestxray14_images", default=DATASETS_CONFIG["cls_nih_root"]
            )
    ...
    if args.taskcomponent in ['foundation_x3_pretraining', 'foundation_x4_pretraining', 'foundation_x5_pretraining']:       
        ...
        data_dir = _dataset_location(
            "classification", "chexpert", "images", default=DATASETS_CONFIG["cls_chexpert_root"]
        )
        ...
        data_dir = _dataset_location(
            "classification", "vindr_cxr", "images", default=DATASETS_CONFIG["cls_vindrcxr_root"]
        )
        ...
        data_dir = _dataset_location(
            "classification", "shenzhen", "images", default=DATASETS_CONFIG["cls_shenzencxr_root"]
        )
        ...
        data_dir = _dataset_location(
            "classification", "mimic_cxr", "images", default=DATASETS_CONFIG["cls_mimiccxr_root"]
        )
        ...
        data_dir = _dataset_location(
            "classification", "tbx11k", "images", default=DATASETS_CONFIG["cls_tbx11k_root"]
        )
        ...
        data_dir = _dataset_location(
            "classification", "node21", "images", default=DATASETS_CONFIG["cls_node21_root"]
        )
        ...
        ## -- ChestX-Det Dataset -- ##
        data_dir = _dataset_location(
            "classification", "chestx_det", "images", default=DATASETS_CONFIG["cls_chestxdet_root"]
        )
        train_list = _dataset_location(
            "classification", "chestx_det", "splits", "train_json", default=DATASETS_CONFIG["cls_chestxdet_trainList"]
        )
        test_list = _dataset_location(
            "classification", "chestx_det", "splits", "test_json", default=DATASETS_CONFIG["cls_chestxdet_testList"]
        )
    ...
    if args.taskcomponent in ["photographic_datasets"]:
        ...
        ## -- COCO Dataset -- ##
        dataset_train_coco = build_dataset(image_set='train', args=args) ## COCO dataset
        ...
        if args.distributed:
            ...
            # sampler_test = torch.utils.data.SequentialSampler(dataset_test) # added by Nahid
        batch_sampler_train = torch.utils.data.BatchSampler(sampler_train_coco, args.batch_size, drop_last=True)
        train_loader_coco = DataLoader(dataset_train_coco, batch_sampler=batch_sampler_train, collate_fn=utils.collate_fn, num_workers=args.num_workers) # RETURN
        test_loader_coco = DataLoader(dataset_val_coco, 1, sampler=sampler_val, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers) # RETURN
        
        ## -- Objects365 Dataset -- ##
        dataset_train_objects365 = build_dataset(image_set='objects365_train', args=args)
        ...
        if args.distributed:
            ...
            sampler_train_objects365 = torch.utils.data.RandomSampler(dataset_train_objects365)
        ...
        train_loader_objects365 = DataLoader(dataset_train_objects365, batch_sampler=batch_sampler_train, collate_fn=utils.collate_fn, num_workers=args.num_workers)
        test_loader_objects365 = DataLoader(dataset_test_objects365, 1, sampler=sampler_val, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
        ...
        if args.distributed:
            ...
            sampler_train_widerface = torch.utils.data.RandomSampler(dataset_train_widerface)
        ...
        if args.batch_size > 6:
            batch_sampler_train = torch.utils.data.BatchSampler(sampler_train_widerface, 6, drop_last=True)
        else:
            batch_sampler_train = torch.utils.data.BatchSampler(sampler_train_widerface, args.batch_size, drop_last=True)
        train_loader_widerface = DataLoader(dataset_train_widerface, batch_sampler=batch_sampler_train, collate_fn=utils.collate_fn, num_workers=args.num_workers)
        test_loader_widerface = DataLoader(dataset_val_widerface, 1, sampler=sampler_val, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)

        return train_loader_coco, test_loader_coco, dataset_val_coco, sampler_train_coco, train_loader_objects365, test_loader_objects365, dataset_test_objects365, sampler_train_objects365, \
        train_loader_widerface, test_loader_widerface, dataset_val_widerface, sampler_train_widerface
    ...
    if args.taskcomponent in ["benchmark"]:
        ...
        if args.dataset_file == "imagenet":
            ...
            train_dataset = datasets.ImageFolder(root=train_dir, transform=build_transform_classification(normalize="imagenet", mode="train"))
            ...
            test_dataset = datasets.ImageFolder(root=val_dir, transform=build_transform_classification(normalize="imagenet", mode="test2"))
            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
        ...
        if args.dataset_file == "compcars":
            ...
            train_dataset = CompCarsDataset(img_dir, train_list, transform=build_transform_classification(normalize="imagenet", mode="train"))
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

            test_dataset = CompCarsDataset(img_dir, val_list, transform=build_transform_classification(normalize="imagenet", mode="test2"))
            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

            return train_loader, test_loader

        ## --- Tsinghua Dogs Classification --- ##
        if args.dataset_file == "tsinghuadogs":
            img_dir = '/ocean/projects/med230002p/nalberti/data/Tsinghua_Dogs/low-resolution'
            ...
            train_dataset = TsinghuaDogsDataset(img_dir, train_lst, transform=build_transform_classification(normalize="imagenet", mode="train"))
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

            test_dataset = TsinghuaDogsDataset(img_dir, train_lst, transform=build_transform_classification(normalize="imagenet", mode="test2"))
            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        ...
        if args.dataset_file == "coco-stuff":
            cocostuff_train_imgs = "/scratch/nralbert/research/data/coco-stuff/train2017"
            ...
            train_dataset = COCOSegmentationDataset(root_dir = cocostuff_train_imgs, annotation_dir = cocostuff_train_imgs, mode = "train")
            sampler = torch.utils.data.RandomSampler(train_dataset)
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.num_workers, pin_memory=True, sampler=sampler)

            test_dataset = COCOSegmentationDataset(root_dir = cocostuff_val_imgs, annotation_dir = cocostuff_val_imgs, mode = "test")
            sampler = torch.utils.data.SequentialSampler(test_dataset)
            test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, sampler=sampler, num_workers=args.num_workers, pin_memory=True)
...
############## ===========  Cls - Loc - Seg DataLoader ============ ################################## -- NAD's code for Dataloader
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning', 'foundation_x4_pretraining', 'foundation_x5_pretraining'] and args.cyclictask == 'segFT_TESTseg_siimacrSEG':  
        ...
        pred_file = os.path.join("/scratch/nuislam/Model_Checkpoints/IntegratedModel_DINOpipeline/IntegratedModel_FoundationX3/TEST_SIIM-ACR/", "prediction")
        ...
        ptx_stage2_images = _dataset_location(
            "directories", "ptx_stage2_images", default="/scratch/sejong/class-dataset/Pneumothorax_segmentation/stage_2_images/"
        )
        dataset_test = SIIMPneumothorax_test_original(images_path=ptx_stage2_images, image_file="stage_2_sample_submission.csv" , image_size=(224,224))
        test_loader_seg_SIIM = torch.utils.data.DataLoader(dataset_test, batch_size=8, shuffle=False, num_workers=8)
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_chexpertCLS':
        train_list = _dataset_location("classification", "chexpert", "splits", "train", default=DATASETS_CONFIG["cls_chexpert_trainList"])
        val_list = _dataset_location("classification", "chexpert", "splits", "val", default=DATASETS_CONFIG["cls_chexpert_valList"])
        test_list = _dataset_location("classification", "chexpert", "splits", "test", default=DATASETS_CONFIG["cls_chexpert_testList"])
        # Original SOL path: /data/jliang12/jpang12/dataset/CheXpert-v1.0/
        data_dir = _dataset_location("directories", "chexpert_v1_root", default=DATASETS_CONFIG["cls_chexpert_root"])
        dataset_train = CheXpert(images_path=data_dir, file_path=train_list, augment=build_transform_classification(normalize="chestx-ray", mode="train"), annotation_percent=100, mode='train')
        dataset_test = CheXpert(images_path=data_dir, file_path=test_list, augment=build_transform_classification(normalize="chestx-ray", mode="test2"), mode='test')
        train_loader_cls_CheXpert = DataLoader(dataset=dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        test_loader_cls_CheXpert = DataLoader(dataset=dataset_test, batch_size=args.batch_size//2, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_vindrcxrCLS':
        train_list = _dataset_location("classification", "vindr_cxr", "splits", "train", default=DATASETS_CONFIG["cls_vindrcxr_trainList"])
        val_list = _dataset_location("classification", "vindr_cxr", "splits", "val", default=DATASETS_CONFIG["cls_vindrcxr_valList"])
        test_list = _dataset_location("classification", "vindr_cxr", "splits", "test", default=DATASETS_CONFIG["cls_vindrcxr_testList"])
        data_dir = _dataset_location("classification", "vindr_cxr", "images", default=DATASETS_CONFIG["cls_vindrcxr_root"])
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_nihshenzenCLS':
        train_list = _dataset_location("classification", "shenzhen", "splits", "train", default=DATASETS_CONFIG["cls_shenzencxr_trainList"])
        val_list = _dataset_location("classification", "shenzhen", "splits", "val", default=DATASETS_CONFIG["cls_shenzencxr_valList"])
        test_list = _dataset_location("classification", "shenzhen", "splits", "test", default=DATASETS_CONFIG["cls_shenzencxr_testList"])
        data_dir = _dataset_location("classification", "shenzhen", "images", default=DATASETS_CONFIG["cls_shenzencxr_root"])
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_mimic2CLS':
        train_list = _dataset_location("classification", "mimic_cxr", "splits", "train", default=DATASETS_CONFIG["cls_mimiccxr_trainList"])
        val_list = _dataset_location("classification", "mimic_cxr", "splits", "val", default=DATASETS_CONFIG["cls_mimiccxr_valList"])
        test_list = _dataset_location("classification", "mimic_cxr", "splits", "test", default=DATASETS_CONFIG["cls_mimiccxr_testList"])
        data_dir = _dataset_location("classification", "mimic_cxr", "images", default=DATASETS_CONFIG["cls_mimiccxr_root"])
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_tbx11kCLS':
        ...
        train_list = 'lists/TBX11K_train.txt'
        test_list = 'lists/TBX11K_val.txt'
        data_dir = _dataset_location("classification", "tbx11k", "images", default=DATASETS_CONFIG["cls_tbx11k_root"])
        dataset_train = TBX11KDataset(images_path=data_dir, file_path=train_list, augment=build_transform_classification(normalize="chestx-ray", mode="train"), annotation_percent=100)
        dataset_test = TBX11KDataset(images_path=data_dir, file_path=test_list, augment=build_transform_classification(normalize="chestx-ray", mode="test2"))
        train_loader_cls_TBX11k = DataLoader(dataset=dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        test_loader_cls_TBX11k = DataLoader(dataset=dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_node21CLS':
        ...
        train_list = 'data/node21_dataset/train.txt'
        test_list = 'data/node21_dataset/test.txt'
        data_dir = _dataset_location("classification", "node21", "images", default=DATASETS_CONFIG["cls_node21_root"])
        dataset_train = NODE21(images_path=data_dir, file_path=train_list, augment=build_transform_classification(normalize="chestx-ray", mode="train"), annotation_percent=100)
        dataset_test = NODE21(images_path=data_dir, file_path=test_list, augment=build_transform_classification(normalize="chestx-ray", mode="test2"))
        train_loader_cls_NODE21 = DataLoader(dataset=dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        test_loader_cls_NODE21 = DataLoader(dataset=dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    if args.taskcomponent in ['foundation_x3_FineTuning'] and args.cyclictask == 'clsFT_TESTcls_chestxdetCLS':
        ## -- ChestX-Det Dataset -- ##
        data_dir = _dataset_location("classification", "chestx_det", "images", default=DATASETS_CONFIG["cls_chestxdet_root"])
        train_list = _dataset_location("classification", "chestx_det", "splits", "train_json", default=DATASETS_CONFIG["cls_chestxdet_trainList"])
        test_list = _dataset_location("classification", "chestx_det", "splits", "test_json", default=DATASETS_CONFIG["cls_chestxdet_testList"])
        dataset_train = ChestXDet_cls(images_path=data_dir, file_path="train", augment=build_transform_classification(normalize="chestx-ray", mode="train"), anno_percent=100)
        dataset_test = ChestXDet_cls(images_path=data_dir, file_path="test", augment=build_transform_classification(normalize="chestx-ray", mode="test2"))
        train_loader_cls_ChestXDet = DataLoader(dataset=dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        test_loader_cls_ChestXDet = DataLoader(dataset=dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    if args.taskcomponent in ['foundation_x_anyDataset'] and args.cyclictask == 'locFT_TESTloc_vindrLOC_kaggle14disease':
        dataset_train_loc = build_dataset(image_set='vindrcxr_train', args=args) ## ChestX-Det Dataset - Training A set # chestxdet_train | chestxdet_train_A | chestxdet_train_B
        ...
        batch_sampler_train = torch.utils.data.BatchSampler(sampler_train, args.batch_size, drop_last=True)
        train_loader_loc = DataLoader(dataset_train_loc, batch_sampler=batch_sampler_train, collate_fn=utils.collate_fn, num_workers=args.num_workers) # RETURN
        test_loader_loc = DataLoader(dataset_val_loc, 1, sampler=sampler_val, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers) # RETURN
        return train_loader_loc, test_loader_loc, dataset_val_loc, 0
    
    if args.taskcomponent in ['foundation_x_anyDataset'] and args.cyclictask == 'locFT_TESTloc_chestxdetLOC':
        dataset_train_loc = build_dataset(image_set='chestxdet_train', args=args) ## ChestX-Det Dataset - Training A set # chestxdet_train | chestxdet_train_A | chestxdet_train_B
    ...
    # nihLOC_test
    if args.taskcomponent in ['foundation_x_anyDataset'] and args.cyclictask == 'locFT_TESTloc_nihLOC':
