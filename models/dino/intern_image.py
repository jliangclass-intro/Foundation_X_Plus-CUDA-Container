...
import sys
sys.path.append('/home/nuislam/projects/DINO_Detection/models/dino')
...
class InternImage(nn.Module):
    ...
    def __init__(self,
                 core_op='DCNv3',
                 channels=64, # channels was 64 by default
                 depths=[4, 4, 18, 4], #[3, 4, 18, 5],
        ...
        self.depths = depths
        self.channels = channels
        # self.num_features = [int(channels * 2**(self.num_levels - 1))] # changed by Nahid -- added third brackets # num_features = [int(embed_dim * 2 ** i) for i in range(self.num_layers)]
    ...
    def forward(self, tensor_list: NestedTensor):
        # print("[CHECK-inside InternImage forward] X", x.shape)
        # exit()
        ...
        for level_idx, level in enumerate(self.levels):
            x, x_ = level(x, return_wo_downsample=True)
            # print("[Inside Backbone] X, X_ Shape:", x.shape, x_.shape)
            # _, H, W = x_.shape # For Testing
            # print("CHECK  ----- ", x.shape, x_.shape)
            _, H, W, _ = x_.shape # For Training

            if level_idx in self.out_indices:
                norm_layer = getattr(self, f'norm{level_idx}')
                x_out = norm_layer(x_)

                out = x_out.view(-1, H, W, self.num_features[level_idx]).permute(0, 3, 1, 2).contiguous()
...
### Added by Nahid
def build_internimage_transformer(modelname, pretrain_img_size=None, **kw):
    assert modelname in ['internimage_T_224_1k', 'internimage_B_224_22k', 'internimage_B_384_22k', 'internimage_L_224_22k', 'internimage_L_384_22k']
    ...
    model_para_dict = {
        ...
        'internimage_B_224_22k': dict(
            embed_dim=128,
            depths=[ 2, 2, 18, 2 ],
            num_heads=[ 4, 8, 16, 32 ],
            window_size=7
        ),
        'internimage_B_384_22k': dict(
            embed_dim=128,
            depths=[ 2, 2, 18, 2 ],
            num_heads=[ 4, 8, 16, 32 ],
            window_size=12
        ),
        'internimage_L_224_22k': dict(
            embed_dim=192,
            depths=[ 2, 2, 18, 2 ],
            num_heads=[ 6, 12, 24, 48 ],
            window_size=7
        ),
        'internimage_L_384_22k': dict(
            embed_dim=192,
            depths=[ 2, 2, 18, 2 ],
            num_heads=[ 6, 12, 24, 48 ],
            window_size=12
