...
import time
import psutil # added for memory leak debugging
import gc
from memory_profiler import profile, memory_usage # added for memory leak debugging
import memory_profiler
import objgraph # used for memory leak debugging
from tqdm import tqdm

# tsne = TSNE(n_components=2, perplexity=30, n_iter=300)

# in scikit-learn v1.5+, n_iter --> max_iter
...
# def train_one_epoch_v2(model: torch.nn.Module, criterion: torch.nn.Module,
#                     data_loader: Iterable, optimizer: torch.optim.Optimizer,
#                     device: torch.device, epoch: int, max_norm: float = 0,
#                     DetHead=None, train_type='uF', wo_class_error=False, lr_scheduler=None, args=None, logger=None, model_ema=None, momen=None, coff=None, criterionMSE=None):
#     import copy
...
#     import sys
#     from collections import OrderedDict
#     import utils  # assuming your utils module includes reduce_dict, etc.
...
#     progress_bar = tqdm(enumerate(data_loader), total=len(data_loader), desc=epoch_desc)
...
#             "class_err": f"{loss_dict_reduced.get('class_error', 0.0):.2f}"
#         })
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, DetHead=None, wo_class_error=False, args=None, logger=None):
    with torch.no_grad():    
        ...
        for samples, targets in metric_logger.log_every(data_loader, 1500, header, logger=logger):
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                if args.tsneOut:
                    ...
                    #if DetHead != 1:
                        #if _cnt == 0:
                            #print("skipping feature detection list...")
            ...
            # periodically delete coco_evaluator to avoid memory issues during Objects365
            if (_cnt+1) % 5000 == 0 and DetHead == 1 and args.taskcomponent in ['photographic_datasets', 'benchmark']:
                #print("--- ## Restarting Cocoeval ## ---")
                ...
                stats_temp = {k: meter.global_avg for k, meter in metric_logger.meters.items() if meter.count > 0}
                if 'bbox' in postprocessors.keys():
                    stats_temp[f'coco_eval_bbox'] = coco_evaluator.coco_eval['bbox'].stats.tolist()
                    #print(f"stats_temp[coco_eval_bbox]: {stats_temp[f'coco_eval_bbox']}")
                if 'segm' in postprocessors.keys():
                    stats_temp[f'coco_eval_masks'] = coco_evaluator.coco_eval['segm'].stats.tolist()
                    #print(f"stats_temp[coco_eval_masks]: {stats_temp[f'coco_eval_masks']}")
                stats_lst.append(stats_temp)
                del coco_evaluator
                coco_evaluator = CocoEvaluator(base_ds, iou_types, useCats=useCats, args=args)
                #if len(stats_lst) > 5:
                    #print("--- ### debug break ### ---")
            ...
            ### DEBUG
            if _cnt % 500 == 0:
                #print(f"Testing: [{_cnt}]")
                """
                print("Intermediate object growth:")
                objgraph.show_growth(limit=10)
                if 'list' in [obj[0] for obj in objgraph.most_common_types(limit=10)]:
                    print("Inspecting lists:")
                    objgraph.show_backrefs(objgraph.by_type('list')[0], max_depth=10, filename='list_backrefs_eval.png')

                if 'dict' in [obj[0] for obj in objgraph.most_common_types(limit=10)]:
                    print("Inspecting dicts:")
                    objgraph.show_backrefs(objgraph.by_type('dict')[0], max_depth=10, filename='dict_backrefs_eval.png')
                    """
            if args.debug:
                if _cnt % 10 == 0:
                    print("BREAK!"*5)
        ...
        if DetHead == 1 and args.taskcomponent in ['photographic_datasets', 'benchmark']:
            return stats_lst
        else:
